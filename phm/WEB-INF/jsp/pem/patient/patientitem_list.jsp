...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="patientitem/list.do" method="post" name="Form" id="Form">
			<table>
				<tr>
						<td>
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								...
								<td style="width: 40px;">${vs.index+1}</td>
										<td>${var.name}</td>
										<%-- <td>${var.hospitalId}</td>
										<td>${var.groupId}</td> --%>
										<td>${var.groupName}</td>
										<td>${var.checkCardNum}</td>										
										<td>
											<c:if test="${var.checkCardType ==1}">实体卡</c:if>
											<c:if test="${var.checkCardType ==2}">虚拟卡</c:if>											
										</td>
										<%-- <td>${var.checkOfficeId}</td> --%>
										<td>${var.checkOfficeName}</td>
										<%-- <td>${var.checkDoctorId}</td> --%>
										<td>${var.checkDoctorName}</td>
										<%-- <td>${var.checkPackageId}</td> --%>
										<td>${var.checkPackageName}</td>
										<td>
											<c:if test="${var.treatCardStatus ==1}">开启</c:if>
											<c:if test="${var.treatCardStatus ==0}">关闭</c:if>	
										</td>
										<td>${var.treatCardNum}</td>

										<td>${var.department}</td>
										<td>
											<c:if test="${var.gender ==1}">男</c:if>
											<c:if test="${var.gender ==0}">女</c:if>	
										</td>
										<td>${var.mobile}</td>
										<td>${var.birthday}</td>
										<td>${var.degree}</td>
