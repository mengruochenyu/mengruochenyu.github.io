...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="patientcheckquestionoption/list.do" method="post" name="Form" id="Form">
			<table>
				<tr>
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								...
								<td style="width: 40px;">${vs.index+1}</td>
										...
										<td>${var.checkSheetId}</td>
										<td>${var.checkSheetName}</td>
										<td>${var.questionId}</td>
										<td>${var.questionDescription}</td>
										<td>${var.questionName}</td>
										<td>${var.patientOption}</td>
										<td>${var.patientScore}</td>
...
<script type="text/javascript">
	...
	function add(){
		 BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>patientcheckquestionoption/goAdd.do'),
            title: '新增'
          });
	}
	...
	function edit(id){
		BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>patientcheckquestionoption/goEdit/'+id),
            title: '编辑'
          });
	}
	...
	//删除
	function del(id){
		var url = "<%=basePath%>patientcheckquestionoption/delete/"+id;
		ajaxDelete(url);
	}
...
<script type="text/javascript">
	...
	//批量操作
	function makeAll(){
		var url = '<%=basePath%>patientcheckquestionoption/deleteBatch';
		ajaxDeleteBatch(url);
	}
