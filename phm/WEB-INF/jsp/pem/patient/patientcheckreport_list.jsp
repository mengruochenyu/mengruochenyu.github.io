...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="patientcheckreport/list.do" method="post" name="Form" id="Form">
			<table>
				<tr>
					...
					<c:when test="${not empty varList}">
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
                                ...
                                <td>${var.checkTime}</td>
                                <td>${var.reportTime}</td>
                                <td>${var.auditDoctorId}</td>
                                <td>${var.auditDoctorName}</td>
                                <td>${var.auditDoctorSign}</td>
                                <td>${var.auditTime}</td>
                                <td>${var.reportNum}</td>
                                <td>${var.checkComment}</td>
                                <td>${var.checkSuggest}</td>
...
<script type="text/javascript">
	...
	function add(){
		 BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>patientcheckreport/goAdd.do'),
            title: '新增'
          });
	}
	...
	function edit(id){
		BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>patientcheckreport/goEdit/'+id),
            title: '编辑'
          });
	}
	...
	//删除
	function del(id){
		var url = "<%=basePath%>patientcheckreport/delete/"+id;
		ajaxDelete(url);
	}
...
<script type="text/javascript">
	...
	//批量操作
	function makeAll(){
		var url = '<%=basePath%>patientcheckreport/deleteBatch';
		ajaxDeleteBatch(url);
	}
