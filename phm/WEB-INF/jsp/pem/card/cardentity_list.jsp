...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="cardentity/list.do" method="post" name="Form" id="Form">
				...
				<tr>
                    <td>
                        <span class="input-icon">
                            <input autocomplete="off" type="text" name="card_num" value="${pd.card_num}" placeholder="卡片编号" />
		...
		<table style="width:100%;">
			<tr>
				<td style="vertical-align:top;">
                    <a class="btn btn-small btn-primary" href="<%=basePath%>cardentity/excel.do?batchId=${cardBatch.id}">导出Excel</a>
...
<script type="text/javascript">
    ...
    function goExcelExport(){
        BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>cardentity/goExcelExport.do'),
            title: '导出excel'
        });
    }
	...
	function add(){
		 BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>cardentity/goAdd.do'),
            title: '新增'
          });
	}
	...
	function edit(id){
		BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>cardentity/goEdit/'+id),
            title: '编辑'
          });
	}
	...
	//删除
	function del(id){
		var url = "<%=basePath%>cardentity/delete/"+id;
		ajaxDelete(url);
	}
...
<script type="text/javascript">
	...
	//批量操作
	function makeAll(){
		var url = '<%=basePath%>cardentity/deleteBatch';
		ajaxDeleteBatch(url);
	}
