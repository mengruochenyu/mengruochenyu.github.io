...
<html lang="en">
	...
	<%@ include file="../../system/admin/top.jsp"%>
        ...
        <script>
            function expandSwift(obj,sheetId){
                var text=$(boj).text();
                if(text=='展开'){
                    var html="";
                    html+="<tr data-target='tr_sheet_"+sheetId+"'>";
                    html+="<td colspan='7'>";
                    html+="<table class='table-sheet-childs'>";
                    $.get(url,function (data) {
                        var varList = data.data;
                        if (varList != null && varList.size() > 0) {
                            $.each(varList, function (index, vs) {
                                 html+="<tr>";
                                 html+="<td style=\"width: 40px;\">"+(vs.index+1)+"</td>";
                                 html+="<td>"+(vs.name)+"</td>";
                                 html+="<td>"+(vs.echartTypeName)+"</td>";
                                 html+="<td>"+(vs.checkSheetName)+"</td>";
                                 html+="<td>"+vs.rangMin +" - "+(vs.rangMax)+"</td>";
                                 html+="<td class=\"content-td\"><span style=\"\" class=\"content\">"+(vs.comment)+"</span><a href=\"javascript:void(0);\" onclick=\"alert($(this).prev().text());\"><span class=\"content-det\">详细</span></a></td>";
                                 html+="<td class=\"content-td\"><span style=\"\" class=\"content\">"+(vs.suggest)+"</span><a href=\"javascript:void(0);\" onclick=\"alert($(this).prev().text());\"><span class=\"content-det\">详细</span></a></td>";
                                 html+="<td style=\"width: 70px;\" class=\"center\">";
                                 html+="<div>";
                                 html+="<div class=\"inline position-relative\">";
                                 html+="<a class='btn btn-mini btn-info' title=\"编辑\" onclick=\"edit('"+(vs.id)+"');\"><i class=\"fa fa-edit\"></i></a>";
                                 html+="<a class='btn btn-mini btn-danger' title=\"删除\" onclick=\"del('"+(vs.id)+"');\"><i class=\"fa fa-trash\"></i></a>";
                                 html+="</div>";
                                 html+="</div>";
                                 html+="</td>";
                                 html+="</tr>";
                    ...
                    html+="</td>";
                    html+="</tr>";
                    var sheet_tr=$(obj).parent();
                    sheet_tr.after(html);
                    $(obj).find("span").text("折叠");
                }
            }
...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
    ...
    <div class="row-fluid">
            ...
            <!-- 检索  -->
            <form action="checkreporttemplate/list.do" method="post" name="Form" id="Form">
            <table>
                <tr>
                    <td>
                        <span class="input-icon">
                            <input autocomplete="off" id="nav-search-input" type="text" name="name" value="${pd.name}" placeholder="请输入试卷名称" />
                    ...
                    <c:when test="${not empty varList}">
                        ...
                        <c:forEach items="${varList}" var="var" varStatus="vs">
                            <tr>
                                ...
                                <td>${var.rangMin} - ${var.rangMax}</td>
                                <td class="content-td"><span style="" class="content">${var.comment}</span><a href="javascript:void(0);" onclick="alert($(this).prev().text());"><span class="content-det">详细</span></a></td>
                                <td class="content-td"><span style="" class="content">${var.suggest}</span><a href="javascript:void(0);" onclick="alert($(this).prev().text());"><span class="content-det">详细</span></a></td>
...
<script type="text/javascript">
	...
	function add(){
		 BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>checkreporttemplate/goAdd.do'),
            title: '新增'
          });
	}
	...
	//删除
	function del(id){
		var url = "<%=basePath%>checkreporttemplate/delete/"+id;
		ajaxDelete(url);
	}
...
<script type="text/javascript">
	...
	//批量操作
	function makeAll(){
		var url = '<%=basePath%>checkreporttemplate/deleteBatch';
		ajaxDeleteBatch(url);
	}
