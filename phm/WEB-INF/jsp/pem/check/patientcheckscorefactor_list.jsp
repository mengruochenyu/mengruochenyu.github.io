...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="patientcheckscorefactor/list.do" method="post" name="Form" id="Form">
			<table>
				<tr>
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								...
								<td style="width: 40px;">${vs.index+1}</td>
										...
										<td>${var.remark}</td>
										<td>${var.sheetId}</td>
										<td>${var.questionIdStr}</td>
										<td>${var.optionIdStr}</td>
...
<script type="text/javascript">
	...
	function add(){
		 BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>patientcheckscorefactor/goAdd.do'),
            title: '新增'
          });
	}
	...
	function edit(id){
		BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>patientcheckscorefactor/goEdit/'+id),
            title: '编辑'
          });
	}
...
<script type="text/javascript">
	...
	//批量操作
	function makeAll(){
		var url = '<%=basePath%>patientcheckscorefactor/deleteBatch';
		ajaxDeleteBatch(url);
	}
