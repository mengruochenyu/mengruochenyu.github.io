...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="xiaoshanpatientinfo/list.do" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
			...
			<table id="table_report" class="table table-hover">
				...
				<thead>
					<tr>
						...
						<th>病人手机号</th>
						<th>经办人</th>
						<th>办理时间（格式：yyyyMMddHHmmss）</th>
						<th>可用标志（Y-可用 N-停用）</th>
						<th>操作</th>
					</tr>
				</thead>
					...
					<c:when test="${not empty varList}">
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
                                ...
                                <td>${var.formalSchooling}</td>
                                <td>${var.nativePlace}</td>
                                <td>${var.certType}</td>
                                <td>${var.certNo}</td>
                                <td>${var.patientClass}</td>
                                <td>${var.company}</td>
                                <td>${var.publicExpenseNo}</td>
                                <td>${var.contractUnit}</td>
...
<script type="text/javascript">
	...
	function add(){
		 BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>xiaoshanpatientinfo/goAdd.do'),
            title: '新增'
          });
	}
	...
	function edit(id){
		BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>xiaoshanpatientinfo/goEdit/'+id),
            title: '编辑'
          });
	}
	...
	//删除
	function del(id){
		var url = "<%=basePath%>xiaoshanpatientinfo/delete/"+id;
		ajaxDelete(url);
	}
...
<script type="text/javascript">
	...
	//批量操作
	function makeAll(){
		var url = '<%=basePath%>xiaoshanpatientinfo/deleteBatch';
		ajaxDeleteBatch(url);
	}
