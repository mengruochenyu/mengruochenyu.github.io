...
<script type="text/javascript">
	...
	function save(target){
		...
		var action = '${msg}';
		var url = 'hospitaloffice/' + action+"?departmentId=<%=request.getParameter("departmentId")%>";
		ajaxSave(target, url, action,function(dataCode){
			if(dataCode == 625){
				BootstrapDialog.show({title:"保存失败",message:"该账号已存在"});
			}
		},null,function(hospitalOffice){
            $("#officesTD").append("<label><input type=\"checkbox\" name=\"officeIds\" checked=\"checked\" value='"+hospitalOffice.id+"'>"+hospitalOffice.name+"</label>")
            hideDialog(target);
		});
		return false;
	}
...
<body>
	<form action="hospitaloffice/${msg}.do" name="editForm" id="editForm" method="post">
		<input type="hidden" name="id" id="id" value="${entity.id}"/>
		<input type="hidden" name="hospitalId" id="hospitalId" value="${department.hospitalId}"/>
		...
		<table id="table_report" class="table noline">
			...
			<tr>
				...
				<td><input type="text" name="accountName" id="accountName" <c:if test="${msg == 'edit'}">disabled</c:if> value="${entity.accountName}" maxlength="32" placeholder="请输入账号"/></td>
			</tr>
			<tr>
