...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<form action="hospitaloffice/list.do" method="post" name="Form" id="Form">
                <b style="font-size: 14px;">${hospital.name}-${department.name}</b>科室列表
					...
					<c:when test="${not empty varList}">
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
                                ...
                                <td><a href="<%=basePath%>hospitaloffice/detail.do?officeId=${var.id}">${var.name}</a></td>
		...
		<table style="width:100%;">
			<tr>
				<td style="vertical-align:top;">
					<a class="btn btn-small btn-success" onclick="editOffice('${department.id}');">选择科室</a>
                    <a href="javascript:void(0);" class="btn btn-small btn-info" onclick="window.history.back(-1);">返回部门列表</a>
				</td>
...
<script type="text/javascript">
	...
	function editOffice(departmentId) {
        BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>hospitaldepartofficeasso/goEditOffice/'+departmentId),
            title: '选择科室    '
        });
    }
	...
	function add(){
		 BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>hospitaloffice/goAdd.do?departmentId=${pd.departmentId}'),
            title: '新增科室'
          });
	}
	...
	function edit(id){
		BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>hospitaloffice/goEdit/'+id),
            title: '编辑'
          });
	}
	//编辑科室的部门归属
	function editDepartment(id){
		BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>hospitaldepartofficeasso/goEditDepartment/'+id),
            title: '编辑'
          });
	}
	...
	//删除
	function del(id){
		var url = "<%=basePath%>hospitaloffice/delete/"+id;
		ajaxDelete(url);
	}
...
<script type="text/javascript">
	...
	//批量操作
	function makeAll(){
		var url = '<%=basePath%>hospitaloffice/deleteBatch';
		ajaxDeleteBatch(url);
	}
	//修改密码
