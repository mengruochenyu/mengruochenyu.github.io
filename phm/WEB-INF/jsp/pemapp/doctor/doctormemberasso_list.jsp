...
<div id="page-content" class="clearfix">
  <div class="row-fluid">
	<div class="row-fluid">
			<!-- 检索  -->
			<form action="doctormemberasso/list.do" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
...
<script type="text/javascript">
	...
	function add(){
		 BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>doctormemberasso/goAdd.do'),
            title: '新增'
          });
	}
	//修改
	function edit(id){
		BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>doctormemberasso/goEdit/'+id),
            title: '编辑'
          });
	}
	//删除
	function del(id){
		var url = "<%=basePath%>doctormemberasso/delete/"+id;
		ajaxDelete(url);
	}
</script>
...
<script type="text/javascript">
	...
	//批量操作
	function makeAll(){
		var url = '<%=basePath%>doctormemberasso/deleteBatch';
		ajaxDeleteBatch(url);
	}
