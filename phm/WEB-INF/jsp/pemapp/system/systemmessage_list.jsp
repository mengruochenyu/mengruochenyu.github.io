					...
					<c:when test="${not empty varList}">
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								...
								<td style="width: 500px;" class="center">
									<div>
										<c:if test="${QX.edit != 1 && QX.del != 1 }">
										<span class="label label-large label-grey arrowed-in-right arrowed-in"><i class="fa fa-lock" title="无权限"></i></span>
										</c:if>
										<div class="inline position-relative">
										    <a class='btn btn-mini btn-info' title="发送" onclick="sendDoctor('${var.id}');">发送给医生</a>
										    <a class='btn btn-mini btn-info' title="发送" onclick="sendMember('${var.id}');">发送给用户</a>
										    <a class='btn btn-mini btn-info' title="发送" onclick="sendAll('${var.id}');">发送所有人</a>
											<c:if test="${QX.edit == 1 }">
...
<script type="text/javascript">
	...
	function add(){
		 BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>systemmessage/goAdd.do'),
            title: '新增'
          });
	}
	...
	//修改
	function sendAll(id){
		bootbox.confirm("确定发送给所有人吗？", function(result){ 
			if(result){
				$.ajax({
					url: '<%=basePath%>systemmessage/all/send/'+ id,
					type: "POST",
					data: {},
					dataType: "json",
	...
	function sendDoctor(id){
		bootbox.confirm("确定发送给所有医生吗？", function(result){ 
			if(result){
				$.ajax({
					url: '<%=basePath%>systemmessage/doctor/send/'+ id,
					type: "POST",
					data: {},
					dataType: "json",
					success: function(data) {
	...
	//修改
	function sendMember(id){
		bootbox.confirm("确定发送给所有用户吗？", function(result){ 
			if(result){
				$.ajax({
					url: '<%=basePath%>systemmessage/member/send/'+ id,
					type: "POST",
					data: {},
					dataType: "json",
					success: function(data) {
	...
	function edit(id){
		BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>systemmessage/goEdit/'+id),
            title: '编辑'
          });
	}
	...
	//删除
	function del(id){
		var url = "<%=basePath%>systemmessage/delete/"+id;
		ajaxDelete(url);
	}
...
<script type="text/javascript">
	...
	//批量操作
	function makeAll(){
		var url = '<%=basePath%>systemmessage/deleteBatch';
		ajaxDeleteBatch(url);
	}
