...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="systemversion/list.do" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
					...
					<c:when test="${not empty varList}">
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
                                ...
                                <td>${var.version}</td>
                                <td>${var.type}</td>
                                <td>${var.fileDownAddress}</td>
                                <%-- <td>${var.qrcodeDownAddress}</td> --%>
                                <td>${var.updateHint}</td>
                                <td>${var.updateDate}</td>
                                <td>${var.status == 1?"是":"否"}</td>
                                <td>${var.size}</td>
                                <td>${var.isConstraint == 1?"是":"否"}</td>
                                <td>${var.versionType}</td>
...
<script type="text/javascript">
	...
	function add(){
		 BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>systemversion/goAdd.do'),
            title: '新增'
          });
	}
	...
	function edit(id){
		BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>systemversion/goEdit/'+id),
            title: '编辑'
          });
	}
	...
	//删除
	function del(id){
		var url = "<%=basePath%>systemversion/delete/"+id;
		ajaxDelete(url);
	}
...
<script type="text/javascript">
	...
	//批量操作
	function makeAll(){
		var url = '<%=basePath%>systemversion/deleteBatch';
		ajaxDeleteBatch(url);
	}
