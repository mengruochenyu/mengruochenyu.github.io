...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="systemsetting/list.do" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
					...
					<c:when test="${not empty varList}">
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
                                ...
                                <td>${var.name}</td>
                                <td>${var.code}</td>
                                <td>${var.stringValue}</td>
                                <%-- <td>${var.doubleValue}</td>
                                <td>${var.intValue}</td> --%>
                                <td>${var.index}</td>
                               <%--  <td>${var.content}</td> --%>
                                <td>${var.remark}</td>
...
<script type="text/javascript">
	...
	function add(){
		 BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>systemsetting/goAdd.do'),
            title: '新增'
          });
	}
	...
	function edit(id){
		BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>systemsetting/goEdit/'+id),
            title: '编辑'
          });
	}
	...
	//删除
	function del(id){
		var url = "<%=basePath%>systemsetting/delete/"+id;
		ajaxDelete(url);
	}
...
<script type="text/javascript">
	...
	//批量操作
	function makeAll(){
		var url = '<%=basePath%>systemsetting/deleteBatch';
		ajaxDeleteBatch(url);
	}
