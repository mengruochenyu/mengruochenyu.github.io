...
<div id="page-content" class="clearfix">
  <div class="row-fluid">
	<div class="row-fluid">
			<!-- 检索  -->
			<form action="systempackagetype/list.do" method="post" name="Form" id="Form">
			<table>
				<tr><td style="padding-right: 12px;">
	                  <span class="">
	                      <select name="query_key_parentId" id="query_key_parentId">
	                          <option value="" selected = "selected">按照指定类型筛选</option>
	                          <c:choose>
	                              <c:when test="${not empty bigTypeList}">
	                              		<option value="0" <c:if test="${'0'==pd.query_key_parentId}">selected</c:if>>只显示第一级分类</option>
	                                  <c:forEach items="${bigTypeList}" var="packageType">
	                                      <option value="${packageType.id}" <c:if test="${packageType.id==pd.query_key_parentId}">selected</c:if>>${packageType.name}</option>
	                                   </c:forEach>
	                              </c:when>
	                          </c:choose>
	                      </select>
					...
					<td><span class="input-icon">
							<input autocomplete="off" id="nav-search-input" type="text" name="query_key" value="${pd.query_key}" placeholder="这里输入类型名称" />
			...
			<table id="table_report" class="table table-hover">
				<thead>
					<tr><th>
						...
						<th>类型名称</th>
						<th style="width: 30%;">上级类型&nbsp;<em>*</em><span style="font-size: 10px;color: gray;">(为空表示该类型为一级类型)</span></th>
						<th>备注</th>
						<th>操作</th>
					</tr>
				</thead>
					...
					<c:when test="${not empty varList}">
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr><td style="width: 30px;">
								...
								<td>${vs.index+1}</td>
                                <td>${var.name }</td>
                                <td><c:if test="${not empty bigTypeList}">
	                            		<c:forEach items="${bigTypeList}" var="packageType">
	                                      <c:if test="${var.parentId == packageType.id}">${packageType.name}</c:if>
	                                   </c:forEach>
	                            	</c:if>
                                </td>
                                <td>${var.remark}</td>
						...
						<c:if test="${QX.cha == 0 }"><tr><td colspan="100" class="center">您无权查看</td></tr></c:if>
					</c:when>
					<c:otherwise>
...
<script type="text/javascript">
	...
	function add(){
		 BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>systempackagetype/goAdd.do'),
            title: '新增'
          });
	}
	//修改
	function edit(id){
		BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>systempackagetype/goEdit/'+id),
            title: '编辑'
          });
	}
	//删除
	function del(id){
		var url = "<%=basePath%>systempackagetype/delete/"+id;
		ajaxDelete(url);
	}
...
<script type="text/javascript">
	...
	//批量操作
	function makeAll(){
		var url = '<%=basePath%>systempackagetype/deleteBatch';
		ajaxDeleteBatch(url);
	}
