...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			<!-- 检索  -->
			<form action="memberorder/list.do" method="post" name="Form" id="Form">
			<table>
				<tr><td><span class="">
						...
						<c:forEach items="${orderList}" var="var" varStatus="vs">
							<tr><td style="width: 30px;">
                               ...
                               <%--  <td>${var.sheetNameStr}</td> --%>
                                <td>${var.buildTime}</td>
                                <td>${var.payAmount}</td>
                                <td><c:forEach items="${statuslist}" var="status" varStatus="vs">
                                		<c:if test="${status.key == var.status }">${status.value}</c:if>
                                	</c:forEach>
                                </td>
                                <td>${var.payAccount}</td>
                                <%-- <td>${var.payTradeNo}</td> --%>
                                <td>${var.payTime}</td>
                                <td>${var.auditTime}</td>
                                <td>
                                	<c:if test="${var.drawbackApplicationTime == null || var.drawbackApplicationTime == '' }">无退款记录</c:if>
                                	${var.drawbackApplicationTime}</td>
                                <%-- <td>${var.drawbackFinishTime}</td> --%>
								<td style="width: 160px;" class="center">
									<div>
										<a class='btn btn-mini btn-info' title="头衔" onclick="showIntroduce('${var.id}');">查看订单详情</a>
										<c:if test="${QX.edit != 1 && QX.del != 1 }">
										<span class="label label-large label-grey arrowed-in-right arrowed-in"><i class="fa fa-lock" title="无权限"></i></span>
										</c:if>
										<div class="inline position-relative">
											<c:if test="${QX.edit == 1 }">
												<c:if test="${var.status == 6}"><a class='btn btn-mini btn-info' title="退款" onclick="edit('${var.id}');">退款</a></c:if>
											</c:if>
										</div>
									</div>
								</td>
...
<script type="text/javascript">
	...
	function showIntroduce(id){
		BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>memberorder/showorderdetail/'+id),
            title: '订单详情'
          });
	}
